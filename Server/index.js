...
app.post("/tbl_users", (req, res) => {
  try {
    mysqlConnection.query(
      ...
      [req.body.name, req.body.nickname, req.body.password, req.body.rooms],
      (err, rows, fields) => {
...
app.post("/tbl_rooms", (req, res) => {
  try {
    mysqlConnection.query(
      ...
      [req.body.name, req.body.password, req.body.admin_id, req.body.conn_id],
      (err, rows, fields) => {
...
app.get("/tbl_rooms/:conn_id&:password", (req, res) => {
  try {
    mysqlConnection.query(
      ...
      [req.params.conn_id, req.params.password],
      (err, rows, fields) => {
        if (!err && rows[0]) res.send(rows[0]);
        else res.send("Wrong");
      }
...
//Add Participant In Room TBL_ROOM
app.post("/tbl_rooms", (req, res) => {
  try {
    mysqlConnection.query(
      "UPDATE tbl_rooms SET participant=? WHERE conn_id=? and password=?",
      [req.body.participant, req.body.conn_id, req.body.password],
      (err, rows, fields) => {
        if (!err) res.send(rows[0]);
        else console.log(err);
      }
...
app.get("/tbl_messages/:room_id", (req, res) => {
  mysqlConnection.query(
    "SELECT * FROM tbl_messages WHERE room_id=?",
    [req.params.room_id],
    (err, rows, fields) => {
...
//Show Users TBL_Users
app.get("/tbl_users/:id", (req, res) => {
  mysqlConnection.query(
    "SELECT * FROM tbl_users WHERE id=?",
    [req.params.id],
    (err, rows, fields) => {
      if (!err && rows[0]) res.send([rows[0].id, rows[0].name]);
